	...
	{
		sharedLine[KERNEL_SIZE + 3] = texture(sampler2D(inImage, linearSampler), vec2(dispatchLocation.xy + ivec2(0,1)) / size);
		sharedLine[KERNEL_SIZE + 4] = texture(sampler2D(inImage, linearSampler), vec2(dispatchLocation.xy + ivec2(0,2)) / size);
		sharedLine[KERNEL_SIZE + 5] = texture(sampler2D(inImage, linearSampler), vec2(dispatchLocation.xy + ivec2(0,3)) / size);
	}
